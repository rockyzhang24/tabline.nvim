...
if not ok then
  ...
  icons.fzf =
    { icon = '', color = '#d0bf41', cterm_color = '185', name = 'fzf' }
  icons.python =
    { icon = '', color = '#3572A5', cterm_color = '67', name = 'python' }
  icons.default =
    { icon = '', color = '#6d8086', cterm_color = '66', name = 'default' }
end
--}}}
...
local function make_icons_hi(gcol, tcol)
  ...
  for _, v in ipairs(groups) do
    ...
    local c = tgc and gcol:sub(2)
      or tcol
      or require('tabline.term256').hex2term(gcol:sub(2))
    vim.cmd(
      printf('hi T%s%s %sbg=%s %sfg=%s', v, c, gui, bg, gui, tgc and gcol or c)
    )
    ret[v] = {}
...
function M.devicon(b, selected) -- {{{1
  if icons then
    ...
    if icon then
      ...
      local kind = selected and (not s.colored_icons and 'ncl' or 'sel')
        or 'dim'
