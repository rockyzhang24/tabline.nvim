...
local function verify_and_exec(typ, dst) -- verify choice {{{1
  ...
  elseif locdir() == 1 and getcwd() == dst then
    ...
    action = fn.confirm(
      'Same as current window-local directory, keep it or clear it?',
      '&Keep\n&Clear'
    )
    if action == 2 then
      vim.cmd('cd ' .. dst)
  ...
  elseif locdir() == 1 then
    ...
    action = fn.confirm(
      'Overwrite window-local directory ' .. getcwd() .. '?',
      '&Yes\n&Keep\n&Clear'
    )
    if action == 1 then
      vim.cmd('lcd ' .. dst)
  ...
  elseif locdir(-1, 0) == 1 and getcwd(-1, 0) == dst then
    ...
    action = fn.confirm(
      'Same as current tab-local directory, keep it or clear it?',
      '&Keep\n&Clear'
    )
    if action == 2 then
      vim.cmd('cd ' .. dst)
  ...
  elseif locdir(-1, 0) == 1 then
    ...
    action = fn.confirm(
      'Overwrite tab-local directory ' .. getcwd() .. '?',
      '&Yes\n&Keep\n&Clear'
    )
    if action == 1 then
      vim.cmd('tcd ' .. dst)
